...
dp.message_handler(commands=['start'])(handlers.Start.start) # кнопка старт

dp.message_handler(Text(equals=['ℹ️ О нас']))(handlers.Start.about_as)# кнопка о нас

dp.message_handler(Text(equals=['✈️Поехали']))(handlers.Category.incity) # кнопка поехали

dp.message_handler(state= UserState.incity_b)(handlers.Category.handle_city_input) # ввод incity


dp.message_handler(Text(equals=['Подать заявку']))(handlers.Category.zayavka) # КНОПКА подать заявку

# Обработчик Кнопок ввода города вылета
...
dp.message_handler(state= UserState.country_b)(handlers.Category.handle_country_input) # ввод страны

# Обработчик Кнопок ввода страны путешествия
...
#
# Обработчик Кнопок количества взрослых людей
...
# Обработчик Кнопок количества детей
...
# Обработчик Кнопок минимальной категории отеля
...
# Обработчик Кнопок категории питания в отеле
...
# Обработчик Кнопок количества ночей в отеле
...
#обработчик ввода с клавиатуры имени путешественика
dp.message_handler(state= UserState.nochi_b)(handlers.Category.handle_name_input)

#обработчик ввода с клавиатуры email путешественика
dp.message_handler(state= UserState.name_b)(handlers.Category.handle_email_input)

# Обработчик Кнопок
dp.message_handler(Text(equals=['Подать заявку']),state=UserState.email_b)(handlers.Category.zayavka)

dp.message_handler(Text(equals=['Редактировать']),state=UserState.email_b)(handlers.Category.incity)
