...
# from main import date,datetime
...
async def nochi_v(call:types.CallbackQuery, state):
    ...
    with open('files/media/name.jpg', "rb") as img:
        await call.message.answer_photo(img,'<b>Введите контактные данные для обратной связи </b>', parse_mode='HTML')
    data = await state.get_data()
    nochi= data['dc']
    await UserState.nochi_b.set()
...
async def handle_name_input(message: types.Message, state):
    ...
    text = open("files\Zayavka.txt", 'a', encoding='utf-8')
    text.write(f'Контактные данные путешественика {name}\n')
    text.close
    print(f'Контактные данные {name}')
    data = await state.get_data()
    name = data['uc']
    with open('files/media/email.jpg', "rb") as img:
        ...
        await message.answer(f'Контактные данные: {name}')
